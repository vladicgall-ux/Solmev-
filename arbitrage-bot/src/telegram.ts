...
type PendingField = "minspread" | "minvolume" | "interval" | "cooldown";
...
function formatOpportunity(o: ArbitrageOpportunity): string {
  return (
    `${o.base}  +${o.spreadPct.toFixed(2)}%\n` +
    `  купить на ${o.buyExchange}: ${o.buyPrice}\n` +
    `  продать на ${o.sellExchange}: ${o.sellPrice}`
  );
}

function formatList(opps: ArbitrageOpportunity[], limit: number): string {
  if (opps.length === 0) return "Пока ничего не найдено выше порога.";
  return opps.slice(0, limit).map(formatOpportunity).join("\n\n");
}

export function startTelegramBot(scanner: ArbitrageScanner): Telegraf {
  ...
  bot.start((ctx) =>
    ctx.reply(
      "Межбиржевой арбитраж-сканер. Управляй кнопками внизу ⬇️\n\n" +
        `Сравнивает цены пар к ${config.quoteAsset} на биржах: ${config.exchanges.join(", ")}.\n` +
        "Нажми ▶️ Старт, чтобы начать сканирование.",
      mainKeyboard(),
    ),
  );
  ...
  bot.hears(BTN.start, async (ctx) => {
    ...
    scanner.start();
    await ctx.reply(
      `Запущен. Интервал: ${config.pollIntervalSec}с, мин. спред: ${config.minSpreadPct}%, ` +
  ...
  bot.hears(BTN.top, async (ctx) => {
    ...
    const header =
      `Последнее сканирование: ${scanAt.toLocaleTimeString("ru-RU")}` +
  ...
  bot.on("callback_query", async (ctx) => {
    ...
    if (data.startsWith("set:")) {
      const field = data.slice("set:".length) as PendingField;
